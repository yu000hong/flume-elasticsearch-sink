...
@Slf4j
public class HeaderIndexer implements Indexer {
    private static final String INDEX_HEADER = "index.header";
    ...
    private static final String ID_HEADER = "id.header";
    ...
    public String getIndex(Event event) {
        ...
        String index = headers.get(indexHeader);
        if (index == null) {
            index = DEFAULT_INDEX;
        }
        if (includeIndexes != null && !includeIndexes.contains(index)) {
            return null;
        }
